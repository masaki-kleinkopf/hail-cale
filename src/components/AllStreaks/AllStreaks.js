...
const AllStreaks = ({ allTeams }) => {
  ...
  useEffect(() => {
    Promise.all(
      allTeams.map((team) => {
        return fetch(
          `https://statsapi.web.nhl.com/api/v1/teams/${team.id}/?expand=team.schedule.previous`
        ).then((response) => {
          if (!response.ok) {
          } else {
      ...
      let teamData = data.map(
        (teamData) =>
          teamData.teams[0].previousGameSchedule.dates[0].games[0].link
      );
  ...
  useEffect(() => {
    if (lastGameLinks.length > 0) {
      Promise.all(
        lastGameLinks.map((link) => {
          return fetch(`https://statsapi.web.nhl.com${link}`).then(
            (response) => {
              if (!response.ok) {
              } else {
                return response.json();
        ...
        let filteredAllPlayers = allPlayers
          .filter((data, index, self) => {
            return (
              index ===
              self.findIndex((player) => player.person.id === data.person.id)
            );
          })
          .map((data) => {
